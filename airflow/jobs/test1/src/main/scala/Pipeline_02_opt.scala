...
//      .master("local[*]")
    ...
    val basePath = config.getString("app.cell_detect.base_path_2")
    ...
    cellDetectDF.write.format("hudi")
      ...
      .mode(SaveMode.Append) // Overwrite: 2 table co the cung 1 base path HDFS vi ghi de. Append: 2 table phai khac base path
      .save(basePath)
