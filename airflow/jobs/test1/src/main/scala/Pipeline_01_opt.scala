...
//      .replace("{keystore_path}", prestoKeyStorePath)
    ...
    val sipWithKey = sipInvite
      ...
      .repartition(numParts, col("join_key"))
    ...
    val bearerTimeDF = sipWithKey.hint("MERGE").join(ebsTempDF.hint("MERGE"),
      (sipWithKey("join_key") === ebsTempDF("msisdn")) &&
        (
          (col("event_id") === "l_bearer_activate" && col("event_time").between(col("start_ts") - 3, col("start_ts") + 3)) ||
          (col("event_id") === "l_bearer_deactivate" && col("event_time").between(col("end_call_ts") - 3, col("end_call_ts") + 3))
        )
    )
      .drop("msisdn")
      .groupBy("call_id", "start_ts", "end_call_ts", "join_key")
      .agg(
        functions.max(when(col("event_id") === "l_bearer_activate", col("event_time"))).alias("activate_time"),
        functions.min(when(col("event_id") === "l_bearer_deactivate", col("event_time"))).alias("deactivate_time")
      )
